...
class Account:
    ...
    async def get_cats(self) -> list:
        if not self.token:
            if not await self.async_login():
                return []
        api = "token/pet/health/v2/cats/"
        rsp = await self.request(api, {"timeZoneId": ""})
        eno = rsp.get("returnCode", 0)
        if eno == 1002:  # Illegal token
            if await self.async_login():
                rsp = await self.request(api)
        dls = rsp.get("data", {}).get("cats") or []
        if not dls:
...
class LitterBox(Device):
    ...
    def modes(self):
        return {
            ...
            "01": "manual",
            "02": "time",
            # "03": "empty", # Not support on LITTER_BOX_599
        }
